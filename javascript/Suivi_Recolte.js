...
function FindingConformite(list_des_date,date_rec_debut,date_rec_fin){
    ...
    for(let i =0 ; i< list_des_date.length;i++){
        ...
        if(viri1){
            if(date_rec_debut>=list_des_date[i].x && date_rec_debut <=list_des_date[i].y){
                viri1=false;
            }
        }   

        if(viri2){
            if(date_rec_fin>=list_des_date[i].x && date_rec_fin <=list_des_date[i].y){
                viri2=false;
            }
        }   
    }
...
function Actualiser(){
    ...
    for(let v=0 ; v<list_suivi_recolte.length;v++){
        ...
        let date_rec_debut= new Date(`${list_suivi_recolte[v].date_recolte}T${list_suivi_recolte[v].heure_debut_recolte}`)
        let date_rec_fin= new Date(`${list_suivi_recolte[v].date_recolte}T${list_suivi_recolte[v].heure_fin_recolte}`)
        //Get_list_Of_Parcelle(parcelle.value)
        conformite=FindingConformite(Get_list_Of_Parcelle(list_suivi_recolte[v].parcelle),date_rec_debut,date_rec_fin)
